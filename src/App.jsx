...
const INITIAL_MD = [
  ...
  '',
  '**待測試（優先順序）**：',
  '1. **🔥 方案二：阻止深度睡眠** + **方案四：VM 層級調整**（**最優先，組合測試**）',
  '   ```bash',
  '   # Host 端：',
  '   - disable_idle_d3=1（VFIO 模組參數）',
  '   - Runtime PM control = on（hookscript）',
  '   - pcie_aspm.policy=performance（GRUB）',
  '   ',
  '   # VM 端：',
  '   - args: -cpu host,kvm=off,hv-vendor-id=xxx',
  '   - kvm.ignore_msrs=1',
  '   ```',
  '2. 如果仍有問題：測試其他遊戲（3DMark、Cyberpunk 2077）',
  '3. 如果其他遊戲正常：考慮 MSFS 特定設定',
  '',
  '### 監控與診斷',
  '```bash',
  '# 問題發生時收集資訊',
...
const TIMELINE_EVENTS = [
  ...
  },
  {
    ...
    details: [
      ...
      '新策略：組合測試 disable_idle_d3 + KVM hidden + MSR ignore'
    ],
    type: 'milestone',
    icon: AlertCircle
  },
