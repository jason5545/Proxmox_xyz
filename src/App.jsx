...
const INITIAL_MD = [
  ...
  '> **策略**：由於系統已透過黑名單+hookscript 達到穩定，專注於 VM 內部解決方案',
  '',
  '1. **Windows Registry 設定**（最有潛力解決 TOPPS 問題）',
  '2. **NVIDIA Profile Inspector**（Windows 端電源管理）',
  '3. **VFIO 模組參數**（Host 端，需重開機）',
  '4. **VM 層級調整**（KVM 隱藏、MSR 處理）',
  '5. ~~Host 端 nvidia-smi~~（已黑名單，不適用）',
  '',
  '### 監控與診斷',
  '```bash',
  '# 問題發生時收集資訊',
