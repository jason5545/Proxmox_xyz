...
const INITIAL_MD = [
  ...
  '  - ✅ hookscript 正常運作',
  '  - ✅ 顯示記憶體無異常',
  '- **問題本質**：GPU 電源狀態轉換（P0→P8）在 VFIO 層被攔截，導致狀態機混亂',
  '',
  '### 待測試解決方案',
  '',
  ...
  '',
  '# 或使用 PowerShell 指令',
  'reg add "HKLM\\SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers" /v "DisableIdlePowerManagement" /t REG_DWORD /d 1',
  '```',
  '',
  '#### 方案二：VFIO 模組參數調整',
  '```bash',
  '# /etc/modprobe.d/vfio.conf 新增',
  'options vfio-pci ids=10de:2705,10de:22bb disable_idle_d3=1 nointxmask=1',
  '',
  '# 或嘗試',
  'options vfio-pci ids=10de:2705,10de:22bb enable_sriov=0 disable_idle_d3=1',
  '```',
  '',
  ...
  '',
  '# 核心參數（/etc/default/grub）',
  'GRUB_CMDLINE_LINUX_DEFAULT="... kvm.ignore_msrs=1"',
  '```',
  '',
  '#### 方案五：Host 端強制效能模式',
  '```bash',
  '# ⚠️ 注意：以下指令針對不同 GPU 廠牌',
  '',
  '# AMD/Radeon GPU（DRM 介面）',
  'echo "high" > /sys/class/drm/card0/device/power_dpm_force_performance_level',
  '',
  '# NVIDIA GPU（如果 Host 有載入 nvidia 驅動）',
  'nvidia-smi -pm 1  # 啟用持續模式',
  'nvidia-smi -pl 400  # 設定功率限制（範例：400W）',
  '```',
  '',
  '### 測試順序建議',
  '1. **先試 Windows Registry**（最簡單、風險最低）',
  '2. **再試 NVIDIA Profile Inspector**（Windows 端調整）',
  '3. **接著 VFIO 模組參數**（需要重開機）',
  '4. **最後考慮 Host 端調整**（影響範圍較大）',
  '',
  '### 監控與診斷',
  '```bash',
  '# 問題發生時收集資訊',
  'dmesg -T | grep -E "vfio|BAR|reset|TOPPS" > /tmp/topps_debug.log',
